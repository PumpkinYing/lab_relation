...
def print_pic(output, out, lr, layer, lambd) :
    plt.figure()
    plt.subplot(2,1,1)
    ...
    plt.legend(loc=3)
    plt.savefig('./pics/result_%f_lr_%d_layer_%f_lambda.png'%(lr, layer, lambd))
...
for layer in layers :
    ...
    print_pic(output, out, min_lr, layer, 0.1)
    if(cur_loss < min_loss) :
        min_loss = cur_loss
        min_layer = layer
...
for lambd in lambdas :
    ...
    print_pic(output, out, min_lr, min_layer, lambd)
    if(cur_loss < min_loss) :
        min_loss = cur_loss
        min_lambda = lambd
'''
